...
const ReservationsList: React.FC<ReservationsListProps> = ({
  ...
  const reservationsByDay = useMemo(() => {
    ...
    reservations.forEach(reservation => {
      ...
      const dateKey = reservation.date instanceof Date ? reservation.date.toISOString().split('T')[0] : new Date(reservation.date).toISOString().split('T')[0];
      if (!groups[dateKey]) {
        groups[dateKey] = [];
      }
    ...
    return Object.entries(groups).map(([dateStr, items]) => ({
      ...
      // Utiliser midi pour éviter tout problème de décalage
      items
    }));
  }, [reservations]);
  ...
  return <>
          ...
          {reservationsByDay.length > 0 ? <div className="space-y-4">
              {reservationsByDay.map((dayGroup, groupIndex) => <div key={dayGroup.date.toISOString()}>
                  ...
                  <div className="space-y-3">
                    {dayGroup.items.map(reservation => <div key={reservation.id} className="flex items-center justify-between p-3 rounded-md border hover:bg-accent/10 cursor-pointer transition-colors" onClick={() => handleEdit(reservation)}>
                        <div>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <Button variant="outline" size="sm" className="text-blue-500 hover:bg-blue-50 hover:text-blue-600" onClick={e => {
                    e.stopPropagation();
                    handleEdit(reservation);
                  }}>
                            <Edit className="h-4 w-4" />
                          </Button>
                          <Button variant="outline" size="sm" className="text-destructive hover:bg-destructive/10 hover:text-destructive" onClick={e => {
                    e.stopPropagation();
      ...
      <ReservationEditModal reservation={selectedReservation} open={isEditModalOpen} onOpenChange={setIsEditModalOpen} onReservationUpdated={handleReservationUpdated} />
    </>;
};
